...
%% Task A.b Simplified Case
%simplified com
com_cm = [1043.1;0;0].*in_to_m;
com_sm = [908.2;0;0].*in_to_m;
com_prop = [905.9;0;0].*in_to_m;

%a)find the center of mass as a vector relative to the A frame
r_csmp_simp = (com_cm*m_cm+com_sm*m_sm+com_prop*m_prop)/tot_mass

%b)find total intertia matrix of csm with prop about B at the com

%find inertia for cm
d_cm = com_cm-r_csmp_simp;
I_cm_o = I_cm+m_cm*(d_cm'*d_cm*eye(3)-d_cm*d_cm');

%find inertia for sm
d_sm = com_sm-r_csmp_simp;
I_sm_o = I_sm+m_sm*(d_sm'*d_sm*eye(3)-d_sm*d_sm');

%find inertia for prop
d_prop = com_prop-r_csmp_simp;
I_prop_o = I_prop+m_prop*(d_prop'*d_prop*eye(3)-d_prop*d_prop');

%find total inertia
I_csmp_o_simp = I_cm_o+I_sm_o+I_prop_o
