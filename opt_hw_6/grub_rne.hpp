...
private:
    ...
    void discretize_joint(double h, double mass, double r_width, int* joint_count, int* link_count, Matrix3d* prev_rot);
    VectorXd get_real_taus();
    Vector3d full_deriv_ang_mom(double u, double v, double h, double mu, double r, 
            double u_dot, double v_dot, double u_ddot, double v_ddot,
            Vector3d alpha_prev, Vector3d omega_prev);
    Vector3d get_inertial_term(int link_num, VectorXd uvdot, 
    ...
    VectorXd _mu, _r, _uv, _uv_real, _uvdot_real ,_uvdot, _uvddot, _m_mat, _G, _Cqdot, _qdd, 
            _flexi_link, _h, _report_tau, _discrete_segments;
    MatrixXd _rc_mat, _re_mat, _rot_mat, _rot_ground_mat, _Inertia_mat, 
            _omega_mat, _alpha_mat, _a_c_mat, _a_e_mat, _v_e_mat, _f_mat,
            _tau_mat, _qdot_mat, _qddot_mat, _M, _K_s, _K_d; 
